`1from flask import render_template,redirect,url_for,abort,flash,request,current_app,make_response
...
def user(username):
	...
	posts=pagination.items
	# pagination is a series of pages
...
def edit_profile_admin(id):
	...
	form.location.data=user.locaiton
...
def post(id):
	...
	if form.validate_on_submit():
		...
		# 再次用到了_get_current_object()
		db.session.add(comment)
		flash('Your comment has been published.')
		return redirect(url_for('.post',id=post.id,page=-1))

	page=request.args.get('page',1,type=int)
	# 这个page 从 request.args.get 中取得 
	if page==-1:
...
def edit(id):
	...
	form=PostForm()
	if form.validate_ob_submit():
		post.boduy=form.body.data
		db.session.add(post)
		flash('The post has been updated')
		return redirect(url_for('.post',id=post.id))
	form.body.data=post.body

	return render_template('edit_post.html',id=post.id)
...
@main.route('/post/<int:id>',methods=['GET','POST'])
# this is the route for displaying post and adding comment.
def post(id):
	post=Post.query.get_or_404(id)
	form=CommentForm()
	if form.validate_ob_submit():
...
def follow(username):
	...
	if user is None:
		flash('invalid user.')
		return redirect(url_for('.index'))
	if not user_current.is_following(user):
